...
class SalesInvoiceController extends Controller
{
    ...
    public function store(Request $request, AccountingService $accounting, EntryVisibilityService $visibility)
    {
        ...
        DB::transaction(function () use ($request, $data, $accounting, $visibility) {
            ...
            $invoice = SalesInvoice::create(array_merge($data, [
                ...
                'inter_company_target_company_ids'  => $request->boolean('inter_company_transfer')
                    ? $this->validatedTargetCompanyIds($request, auth()->user()->current_company_id)
                    : null,
            ]));
    ...
    public function update(Request $request, SalesInvoice $sale, AccountingService $accounting, EntryVisibilityService $visibility)
    {
        ...
        DB::transaction(function () use ($request, $data, $sale, $accounting, $visibility) {
            // Load existing items BEFORE any delete
            $sale->load('items');
            $oldValues          = $sale->replicate()->toArray();
            $oldValues['items'] = $sale->items->toArray();

            // 1. Reverse old stock movements and party ledger
            ...
            $sale->update(array_merge($data, [
                ...
                'inter_company_transfer'            => $request->boolean('inter_company_transfer'),
                'inter_company_target_company_ids'  => $request->boolean('inter_company_transfer')
                    ? $this->validatedTargetCompanyIds($request, $sale->company_id)
                    : null,
            ]));

            // 4. Re-build unit pool AFTER items deleted (so current invoice units are free)
            //    Pass null so NO invoice is excluded — the current invoice's items are already gone
            $unitPool = $this->finishedGoodsUnitPool($sale->company_id, null);

            // 5. Store new lines with pre-built pool
            $totals = $this->storeLines($request, $sale, $accounting, $unitPool);
            $sale->update($totals);

            // 6. Re-post party ledger with NEW grand_total
    ...
    /**
     * Build new line-items, move stock, and return totals.
     *
     * @param  array|null  $unitPool  Pre-built pool (pass from update() so we don't rebuild mid-transaction)
     */
    private function storeLines(Request $request, SalesInvoice $invoice, AccountingService $accounting, ?array $unitPool = null): array
    {
        $subtotal    = $tax = $lineDiscount = $totalWeight = 0;

        // Build the pool once; for store() it is built fresh with $invoice->id excluded.
        // For update() the caller passes a pre-built pool where the current invoice is NOT excluded
        // because items were already deleted before storeLines() is called.
        if ($unitPool === null) {
        ...
        foreach ($request->item_id as $i => $itemId) {
            ...
            abort_if(
                $item->track_stock && (float) $item->current_stock < $qty,
                422,
                "Insufficient stock for {$item->name}"
            );
            abort_if(
                $item->productType?->nature !== 'finished_goods',
                422,
                'Only finished goods can be sold from Sales.'
            );

            // Decode & normalize selected units against the (fresh) pool
            $rawSelected   = $this->decodeSelectedUnits($request->selected_units[$i] ?? null);
            $selectedUnits = $this->normalizeSelectedUnits($rawSelected, $unitPool[$item->id] ?? []);

            abort_if(
                $this->isGpsItem($item) && collect($selectedUnits)->contains(fn($u) => empty($u['vts_sim'])),
                422,
            ...
            // Available keys = units in pool that are NOT marked sold
            ...
            abort_if(
                count(array_diff($selectedKeys, $availableKeys)) > 0,
                422,
                "One or more selected units for {$item->name} are already sold or invalid."
            );

            // Mark the selected keys as sold inside the in-memory pool to prevent
            // double-selection within the same invoice (multiple lines, same item).
            ...
            $base               = $qty * $price;
            $discountType       = $request->discount_type[$i] ?? 'percent';
            $discountValue      = (float) ($request->discount_value[$i] ?? 0);
            $discount           = $discountType === 'flat' ? $discountValue : $base * $discountValue / 100;
    ...
    /**
     * Build a map of item_id => unit[] combining produced and purchased finished goods.
     *
     * @param  int|null  $currentInvoiceId  When NOT null, units sold on THIS invoice are
     *                                       treated as available (used for the edit-form pool).
     *                                       Pass null when units have already been freed
     *                                       (i.e. items deleted before calling storeLines).
     */
    private function finishedGoodsUnitPool(int $companyId, ?int $currentInvoiceId = null): array
    {
        // Collect all unit keys that are already sold, optionally excluding the current invoice
    ...
    private function createInterCompanyPurchases(SalesInvoice $invoice, AccountingService $accounting, Request $request): void
    {
        ...
        // Remove purchases for companies that are no longer in the target list
    ...
    {
        return str_pad(
            (string) (SalesInvoice::where('company_id', auth()->user()->current_company_id)->withTrashed()->count() + 1),
            8,
            '0',
            STR_PAD_LEFT
        );
