...
<div class="card">
    ...
    <div class="card-body table-responsive">
        ...
        @foreach($returns as $return)<tr><td>{{ $return->return_no }}</td><td>{{ $return->return_date?->format('d M Y') }}</td><td>{{ $return->bill?->invoice_no }}</td><td>{{ $return->party?->display_name ?: 'Cash' }}</td><td>Rs {{ number_format((float)$return->grand_total,2) }}</td><td><a href="{{ route('admin.purchase-returns.show',$return) }}" class="btn btn-info btn-sm"><i class="fas fa-eye"></i></a> @can('purchase.edit')<a href="{{ route('admin.purchase-returns.edit',$return) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>@endcan</td></tr>@endforeach
        </tbody></table>
    </div>
</div>
@endsection
