...
<head>
    ...
    <style>
        *{box-sizing:border-box} body{font-family:Arial,sans-serif;background:#eef4ff;margin:0;padding:18px;color:#0f172a}.sheet{max-width:980px;margin:auto;background:#fff;border:2px solid #111827;border-radius:10px;overflow:hidden}.top{display:flex;justify-content:space-between;align-items:flex-start;padding:18px 22px;background:#f8fbff}.brand-wrap{display:flex;gap:14px;align-items:flex-start}.logo{width:72px;height:72px;object-fit:contain;border:1px solid #dbeafe;border-radius:8px;background:#fff}.brand{font-size:28px;font-weight:800;color:{{ $accent }}}.company-lines{line-height:1.45}.doc-title{background:{{ $accent }};color:#fff;padding:18px 22px;display:flex;justify-content:space-between;align-items:center}.doc-title h1{margin:0;font-size:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;padding:18px 22px}.box{border:1px solid #cbd5e1;border-radius:8px;padding:12px;min-height:110px}.box h3{margin:0 0 8px;color:{{ $accent }};font-size:15px}.meta td{padding:5px 0 5px 18px}.items{width:calc(100% - 44px);margin:8px 22px 18px;border-collapse:collapse}.items th{background:linear-gradient(90deg,#38bdf8,#22d3ee);color:#fff;text-align:left}.items th,.items td{border:1px solid #e2e8f0;padding:9px;font-size:13px}.items tfoot td{font-weight:800;background:#eff6ff}.summary{margin:0 22px 22px auto;width:360px;border:1px solid #2563eb}.summary div{display:flex;justify-content:space-between;padding:9px 12px;border-bottom:1px solid #dbeafe}.summary div:last-child{border-bottom:0;font-size:18px;font-weight:800}.footer{display:grid;grid-template-columns:1.2fr 1.4fr .9fr;gap:0;margin:0 22px 22px;border:1px solid #64748b}.footer>div{min-height:165px;border-right:1px solid #64748b;padding:12px}.footer>div:last-child{border-right:0;text-align:center;padding-top:90px}.qr{width:92px;height:92px;object-fit:contain;margin-top:8px}.muted{color:#64748b;font-size:12px}@media print{body{background:#fff;padding:0}.sheet{border-radius:0;max-width:none}.no-print{display:none}}
    </style>
</head>
<body>
...
<div class="sheet">
    <div class="top">
        <div class="brand-wrap">@if($company?->logo)<img class="logo" src="{{ asset('storage/'.$company->logo) }}" alt="logo">@endif<div><div class="brand">{{ $company?->name ?? config('app.name', 'Eemot Account') }}</div><div class="company-lines muted">{{ $company?->phone }} {{ $company?->email ? '| '.$company->email : '' }}<br>{{ $company?->gst_number ? 'GST: '.$company->gst_number : '' }} {{ $company?->pan_number ? '| PAN: '.$company->pan_number : '' }}<br>{!! nl2br(e($company?->address)) !!}</div></div></div>
    ...
    <div class="footer"><div><b>Bank Details</b><br>@if($bankAccount)<span class="muted">{{ $bankAccount->bank_name }} {{ $bankAccount->branch_name ? '| '.$bankAccount->branch_name : '' }}</span><br>Holder: {{ $bankAccount->account_holder_name ?: $company?->name }}<br>A/C: {{ $bankAccount->account_number ?: '-' }}<br>IFSC: {{ $bankAccount->ifsc_code ?: '-' }}<br>UPI: {{ $bankAccount->upi_id ?: '-' }}@if($bankAccount->upi_qr_code)<img class="qr" src="{{ asset('storage/'.$bankAccount->upi_qr_code) }}" alt="UPI QR">@endif @else <span class="muted">No bank account selected for bill print.</span>@endif</div><div><b>Terms & Conditions</b><br>{!! nl2br(e($terms ?? '')) !!}</div><div><b>Authorized Signatory</b></div></div>
</div>
</body>
</html>
