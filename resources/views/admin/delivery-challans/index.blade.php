    ...
    <div class="card-body table-responsive">
        <table id="challansTable" class="table table-hover">
            ...
            @foreach($challans as $challan)
                ...
                <tr><td><strong>{{ $challan->challan_no }}</strong></td><td>{{ $challan->challan_date?->format('d M Y') }}</td><td>{{ $challan->party?->display_name ?: 'Walk-in' }}</td><td>{{ $challan->creator?->name ?? 'System' }}<br><small class="text-muted">{{ $challan->creator?->rolesForCompany($challan->company_id)->pluck('name')->join(', ') }}</small></td><td>{{ $challan->vehicle_no ?: '-' }}</td><td>{{ $challan->dispatch_through ?: '-' }}</td><td><span class="badge-active">{{ ucfirst($challan->status) }}</span></td><td><a href="{{ route('admin.delivery-challans.show', $challan) }}" class="btn btn-info btn-sm"><i class="fas fa-eye"></i></a> @if($canManage && $challan->status !== 'cancelled' && auth()->user()->can('delivery_challans.edit'))<a href="{{ route('admin.delivery-challans.edit', $challan) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i></a>@endif @can('delivery_challans.print')<a href="{{ route('admin.delivery-challans.print', $challan) }}" class="btn btn-secondary btn-sm" target="_blank"><i class="fas fa-print"></i></a>@endcan</td></tr>
            @endforeach
            </tbody>
        </table>
    </div>
